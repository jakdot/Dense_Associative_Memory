...
def draw_weights(synapses, Kx, Ky, err_tr, err_test, filenum="none"):
    ...
    plt.tight_layout()
    fig.canvas.draw()
    plt.savefig(f'images/mnist-smallmemory-results-{filenum}.png')
...
Kx=4              # Number of memories per row on the weights plot
Ky=4              # Number of memories per column on the weigths plot
...
p=0.6              # Momentufm
Nep=50            # Number of epochs
Temp_in=700.       # Initial temperature
Temp_f=500.        # Final temperature
...
with open('trained-hopfield-small-memory.pkl', 'wb') as savefile:
